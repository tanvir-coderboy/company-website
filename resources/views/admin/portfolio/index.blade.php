            ...
            <div class="col-md-12">
                <div class="card card-cyan">
                    <div class="card-header">
                        ...
                        <a href="{{ route('admin.portfolios.create') }}" class="btn btn-success float-right"><i class="fa fa-plus"></i> Add </a>
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @can('edit user')
                                        <a href="{{ route('admin.portfolios.edit', $item->id) }}" class="btn btn-sm btn-secondary"  data-bs-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>
                                        <a href="{{ route('admin.portfolios.edit', $item->id) }}" class="btn btn-sm btn-primary"  data-bs-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i></a>
                                        @endcan
                                        @can('delete user')
                                        <form id="delete-form-{{ $item->id }}" action="{{ route('admin.portfolios.destroy', $item->id) }}" method="POST" style="display: none;">
                                            @csrf @method('DELETE')
                                        </form>
...
<script>
    $(document).ready(function() {
        $('.status-toggle').on('change', function() {
            ...
            $.ajax({
                url: "{{ route('admin.portfolios.status.update') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id,
